...
function Item({ item }) {
  return (
    <TouchableOpacity style={styles.item} onPress={() => navigation.navigate('InfoProduct')}>
      ...
      <View style={styles.title}>
        ...
        <Text style={styles.item_subdivision}>
          Parcelado em 12 x{' '}
          <Text style={styles.item_subdivision_detach}>R${(item.price / 12).toFixed(2)}</Text>
        </Text>
...
export default function List() {
  ...
  return (
    <>
      <Text style={styles.text_results}>Produtos encontrados: {listProduct.length}</Text>
